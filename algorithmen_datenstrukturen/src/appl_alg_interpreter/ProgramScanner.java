	...
	public String nextToken(String regex) throws Exception {
		...
		if (actLine == null || actLine.matches("^\\s*$")) {
			if (regex.equals(ApplAlgParser.NAME))
				...
				String message = "Parsing error in " + filename + " (line " + lineNumber + "): unexpected end of file."; 
				throw new Exception(message);
			}
		}
		...
		if (match == null) {
			String message = "Parsing error in " + filename + " (line " + lineNumber + ") near " + actLine.trim(); 
			throw new Exception(message);
		}
		return match;
	}
